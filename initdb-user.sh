...
psql --set=PG_READ_PWD="$PG_READ_PWD" --set=PG_WRITE_PWD="$PG_WRITE_PWD" --set=GRETL_PG_WRITE_PWD="$GRETL_PG_WRITE_PWD" -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    SET password_encryption = md5;

    CREATE ROLE ddluser LOGIN SUPERUSER PASSWORD :'PG_WRITE_PWD';
    CREATE USER gretl LOGIN ENCRYPTED PASSWORD :'GRETL_PG_WRITE_PWD';
